...
### Modelo de grafos
O modelo de grafos foi construído no Neo4j usando [comandos cypher](src/contrucao_dataset.cypher) para importar os dados das tabelas previamente construídas de acordo com o modelo que já foi apresentado previamente. Foi necessário instalar o Neo4j localmente, pois a implementação em nuvem não tinha capacidade para lidar com a grande quantidade de dados do nosso projeto. O código em cypher responsável pela construção da base de dados está especificado abaixo.
